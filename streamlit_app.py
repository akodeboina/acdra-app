...
# Analyze deferment request with shorter summary
def analyze_deferment_request(client, nric, user_input, pdf_text=""):
    ...
    user_profile = user_profiles.get(nric, {})
    
    # Step 5: Generate concise assessment
    assessment_prompt = f"""
...
def home_page():
    ...
    with col1:
        ...
        # Only Analyze button (Submit button moved to bottom)
        ...
        if analyze_button and nric_valid and user_input:
            with st.spinner("Analyzing your request..."):
                result = analyze_deferment_request(client, nric_input.upper(), user_input, pdf_text)
        ...
        if hasattr(st.session_state, 'current_assessment'):
            ...
            if result["is_deferment"]:
                st.success("✓ Deferment request identified")
            ...
            # Reviewer Comments Section (for both deferment and non-deferment)
            st.divider()
            ...
            # Submit Assessment button at the end
            st.divider()
